	...
	public String[] start(Context ctx, boolean isAutoSign, String cfg, String user, String pwd) {
		...
		try{
			...
			if(isLoginSucceed)
			{
				...
				//获取网盘的token
				res = Jsoup.connect(realTokenUrl).cookies(cookies).userAgent(UA_ANDROID).timeout(TIME_OUT).ignoreContentType(true).method(Method.GET).execute();
				cookies.putAll(res.cookies());
				signinUrl = "http://" + res.url().getHost() + "/user/signin/";
				
				//以上正式完成登录，获取完所有需要的cookies数据，最后要做的就是签到了
				res = Jsoup.connect(signinUrl).data("qid", loginQid).data("method", "signin").cookies(cookies).userAgent(UA_ANDROID).timeout(TIME_OUT).method(Method.POST).ignoreContentType(true).execute();
